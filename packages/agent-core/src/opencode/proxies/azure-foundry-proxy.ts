...
function proxyRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
  ...
  req.on('end', () => {
    ...
    proxy.on('error', (error) => {
      log.error('[Azure Foundry Proxy] Request error:', error);
      if (!res.headersSent) {
  ...
  req.on('error', (error) => {
    log.error('[Azure Foundry Proxy] Incoming request error:', error);
    if (!res.headersSent) {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server!.close((err) => {
      ...
      if (err && (err as NodeJS.ErrnoException).code !== 'ERR_SERVER_NOT_RUNNING') {
        log.error('[Azure Foundry Proxy] Error during shutdown:', err);
        reject(err);
      } else {
