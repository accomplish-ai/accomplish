...
describe('Preload Script Integration', () => {
  ...
  describe('IPC Method Invocations', () => {
    ...
    describe('Shell Operations', () => {
      it('openExternal should invoke shell:open-external with URL', async () => {
        ...
        await (
          capturedAccomplishAPI.openExternal as (url: string) => Promise<void>
        )(url);
    ...
    describe('Task Operations', () => {
      it('startTask should invoke task:start with config', async () => {
        ...
        await (
          capturedAccomplishAPI.startTask as (config: {
            description: string;
          }) => Promise<unknown>
        )(config);
      ...
      it('cancelTask should invoke task:cancel with taskId', async () => {
        await (
          capturedAccomplishAPI.cancelTask as (taskId: string) => Promise<void>
        )('task_123');
      ...
      it('interruptTask should invoke task:interrupt with taskId', async () => {
        await (
          capturedAccomplishAPI.interruptTask as (
            taskId: string
          ) => Promise<void>
        )('task_123');
      ...
      it('getTask should invoke task:get with taskId', async () => {
        await (
          capturedAccomplishAPI.getTask as (taskId: string) => Promise<unknown>
        )('task_123');
      ...
      it('deleteTask should invoke task:delete with taskId', async () => {
        await (
          capturedAccomplishAPI.deleteTask as (taskId: string) => Promise<void>
        )('task_123');
    ...
    describe('Settings Operations', () => {
      ...
      it('setDebugMode should invoke settings:set-debug-mode', async () => {
        await (
          capturedAccomplishAPI.setDebugMode as (
            enabled: boolean
          ) => Promise<void>
        )(true);
        expect(mockInvoke).toHaveBeenCalledWith(
      ...
      it('getAppSettings should invoke settings:app-settings', async () => {
        await (
          capturedAccomplishAPI.getAppSettings as () => Promise<unknown>
        )();
    ...
    describe('API Key Operations', () => {
      ...
      it('setApiKey should invoke api-key:set', async () => {
        await (
          capturedAccomplishAPI.setApiKey as (key: string) => Promise<void>
        )('sk-test');
      ...
      it('getApiKey should invoke api-key:get', async () => {
        await (
          capturedAccomplishAPI.getApiKey as () => Promise<string | null>
        )();
      ...
      it('validateApiKey should invoke api-key:validate', async () => {
        await (
          capturedAccomplishAPI.validateApiKey as (
            key: string
          ) => Promise<unknown>
        )('sk-test');
    ...
    describe('Onboarding Operations', () => {
      it('getOnboardingComplete should invoke onboarding:complete', async () => {
        await (
          capturedAccomplishAPI.getOnboardingComplete as () => Promise<boolean>
        )();
      ...
      it('setOnboardingComplete should invoke onboarding:set-complete', async () => {
        await (
          capturedAccomplishAPI.setOnboardingComplete as (
            c: boolean
          ) => Promise<void>
        )(true);
        expect(mockInvoke).toHaveBeenCalledWith(
    ...
    describe('Model Operations', () => {
      it('getSelectedModel should invoke model:get', async () => {
        await (
          capturedAccomplishAPI.getSelectedModel as () => Promise<unknown>
        )();
    ...
    describe('Logging Operations', () => {
      it('logEvent should invoke log:event', async () => {
        ...
        await (
          capturedAccomplishAPI.logEvent as (p: unknown) => Promise<unknown>
        )(payload);
  ...
  describe('Event Subscriptions', () => {
    it('onTaskUpdate should subscribe to task:update', () => {
      ...
      (
        capturedAccomplishAPI.onTaskUpdate as (
          cb: (e: unknown) => void
        ) => () => void
      )(callback);
    ...
    it('onTaskUpdateBatch should subscribe to task:update:batch', () => {
      const callback = vi.fn();
      (
        capturedAccomplishAPI.onTaskUpdateBatch as (
          cb: (e: unknown) => void
        ) => () => void
      )(callback);
    ...
    it('onPermissionRequest should subscribe to permission:request', () => {
      const callback = vi.fn();
      (
        capturedAccomplishAPI.onPermissionRequest as (
          cb: (e: unknown) => void
        ) => () => void
      )(callback);
      expect(mockOn).toHaveBeenCalledWith(
    ...
    it('onTaskProgress should subscribe to task:progress', () => {
      const callback = vi.fn();
      (
        capturedAccomplishAPI.onTaskProgress as (
          cb: (e: unknown) => void
        ) => () => void
      )(callback);
      expect(mockOn).toHaveBeenCalledWith(
    ...
    it('onDebugLog should subscribe to debug:log', () => {
      const callback = vi.fn();
      (
        capturedAccomplishAPI.onDebugLog as (
          cb: (e: unknown) => void
        ) => () => void
      )(callback);
    ...
    it('onTaskStatusChange should subscribe to task:status-change', () => {
      const callback = vi.fn();
      (
        capturedAccomplishAPI.onTaskStatusChange as (
          cb: (e: unknown) => void
        ) => () => void
      )(callback);
      expect(mockOn).toHaveBeenCalledWith(
