...
test.describe('Settings - Amazon Bedrock', () => {
  test('should display Bedrock provider card', async ({ window }) => {
    ...
    await expect(bedrockCard).toBeVisible({
  ...
  test('should show Bedrock credential form when selected', async ({
    window,
  }) => {
    const settingsPage = new SettingsPage(window);
    await window.waitForLoadState('domcontentloaded');
  ...
  test('should switch between Access Key and AWS Profile tabs', async ({
    window,
  }) => {
    const settingsPage = new SettingsPage(window);
    await window.waitForLoadState('domcontentloaded');
    ...
    await captureForAI(window, 'settings-bedrock', 'tab-switching', [
      'Can switch between auth tabs',
      'Form fields update correctly',
    ]);
  });

  test('should allow typing in Bedrock access key fields', async ({
    window,
  }) => {
    const settingsPage = new SettingsPage(window);
    await window.waitForLoadState('domcontentloaded');
    ...
    await expect(settingsPage.bedrockAccessKeyIdInput).toHaveValue(
      testAccessKey
    );
  ...
  test('should have Connect button for Bedrock credentials', async ({
    window,
  }) => {
    const settingsPage = new SettingsPage(window);
    await window.waitForLoadState('domcontentloaded');
