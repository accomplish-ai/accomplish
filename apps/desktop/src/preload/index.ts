...
const accomplishAPI = {
  ...
  addApiKey: (
    provider: 'anthropic' | 'openai' | 'openrouter' | 'google' | 'xai' | 'deepseek' | 'zai' | 'azure-foundry' | 'custom' | 'bedrock' | 'litellm' | 'elevenlabs',
    key: string,
    label?: string
  ): Promise<unknown> =>
  ...
  speechGetApiKey: (): Promise<string> =>
    ipcRenderer.invoke('speech:get-api-key'),
};
