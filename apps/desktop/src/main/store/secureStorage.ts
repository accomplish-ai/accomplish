 ...
 */
export type ApiKeyProvider = 'anthropic' | 'openai' | 'openrouter' | 'google' | 'xai' | 'deepseek' | 'zai' | 'custom' | 'bedrock' | 'litellm' | 'elevenlabs';

/**
 * Get all API keys for all providers
 */
export async function getAllApiKeys(): Promise<Record<ApiKeyProvider, string | null>> {
  const [anthropic, openai, openrouter, google, xai, deepseek, zai, custom, bedrock, litellm, elevenlabs] = await Promise.all([
    getApiKey('anthropic'),
    getApiKey('openai'),
  ...
  return { anthropic, openai, openrouter, google, xai, deepseek, zai, custom, bedrock, litellm, elevenlabs };
}
