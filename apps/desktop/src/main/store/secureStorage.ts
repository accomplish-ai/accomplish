 ...
 */
export type ApiKeyProvider = 'anthropic' | 'openai' | 'google' | 'xai' | 'groq' | 'custom';

export async function getAllApiKeys(): Promise<Record<ApiKeyProvider, string | null>> {
  const [anthropic, openai, google, xai, groq, custom] = await Promise.all([
    getApiKey('anthropic'),
    getApiKey('openai'),
    getApiKey('google'),
    getApiKey('xai'),
  ...
  return { anthropic, openai, google, xai, groq, custom };
}
