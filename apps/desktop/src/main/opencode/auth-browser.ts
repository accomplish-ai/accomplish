...
export class OAuthBrowserFlow {
  ...
  async start(): Promise<LoginResult> {
    if (this.isInProgress()) {
      try { const l = getLogCollector(); if (l?.log) l.log('INFO', 'opencode', '[OAuthBrowserFlow] Cancelling previous flow before starting new one'); } catch (_e) {}
      await this.cancel();
      try {
        await waitForPortRelease(1455, 2000);
        try { const l = getLogCollector(); if (l?.log) l.log('INFO', 'opencode', '[OAuthBrowserFlow] Port 1455 released'); } catch (_e) {}
      } catch {
        try { const l = getLogCollector(); if (l?.log) l.log('WARN', 'opencode', '[OAuthBrowserFlow] Port 1455 still in use after 2000ms'); } catch (_e) {}
      }
    }
  ...
  async cancel(): Promise<void> {
    if (!this.activePty) {
      try { const l = getLogCollector(); if (l?.log) l.log('INFO', 'opencode', '[OAuthBrowserFlow] No active flow to cancel'); } catch (_e) {}
      return;
    }

    try { const l = getLogCollector(); if (l?.log) l.log('INFO', 'opencode', '[OAuthBrowserFlow] Cancelling active OAuth flow'); } catch (_e) {}
    ...
    if (!gracefulExited && this.activePty === ptyProcess) {
      try { const l = getLogCollector(); if (l?.log) l.log('INFO', 'opencode', '[OAuthBrowserFlow] Force killing after graceful timeout'); } catch (_e) {}
      try {
        ptyProcess.kill();
      } catch (err) {
        try { const l = getLogCollector(); if (l?.log) l.log('WARN', 'opencode', '[OAuthBrowserFlow] Error during force kill', { err: String(err) }); } catch (_e) {}
      }
    }
  ...
  dispose(): void {
    ...
    try { const l = getLogCollector(); if (l?.log) l.log('INFO', 'opencode', '[OAuthBrowserFlow] Disposing'); } catch (_e) {}
    ...
    if (this.activePty) {
      try {
        ...
        try { const l = getLogCollector(); if (l?.log) l.log('WARN', 'opencode', '[OAuthBrowserFlow] Error killing PTY during dispose', { err: String(err) }); } catch (_e) {}
      }
      this.activePty = null;
    }
  }
