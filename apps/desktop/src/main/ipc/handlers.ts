...
function validateTaskConfig(config: TaskConfig): TaskConfig {
  ...
  if (config.workingDirectory) {
    validated.workingDirectory = sanitizeString(
    ...
    async (event: IpcMainInvokeEvent, config: TaskConfig) => {
      ...
      const callbacks: TaskCallbacks = {
        ...
        onComplete: (result: TaskResult) => {
          ...
          const sessionId =
            result.sessionId || taskManager.getSessionId(taskId);
          if (sessionId) {
  ...
  handle(
    ...
    async (_event: IpcMainInvokeEvent, response: PermissionResponse) => {
      ...
      if (requestId && isFilePermissionRequest(requestId)) {
        ...
        if (resolved) {
          console.log(
            `[IPC] File permission request ${requestId} resolved: ${allowed ? 'allowed' : 'denied'}`
          );
          return;
        }
      ...
      if (requestId && isQuestionRequest(requestId)) {
        ...
        if (resolved) {
          console.log(
            `[IPC] Question request ${requestId} resolved: ${denied ? 'denied' : 'answered'}`
          );
          return;
        }
        // If not found in pending, fall through to standard handling
        ...
        switch (provider) {
          ...
          case 'azure-foundry':
            ...
            if (!baseUrl || !deploymentName) {
              console.log(
                '[API Key] Skipping validation for azure-foundry provider (missing config or options)'
              );
              return { valid: true };
            }
            ...
            if (authType === 'entra-id') {
              if (!entraToken) {
                return {
                  valid: false,
                  error:
                    'Missing Entra ID access token for Azure Foundry validation request',
                };
              }
            ...
            if (!response.ok) {
              ...
              const firstErrorMessage =
                (firstErrorData as { error?: { message?: string } })?.error
                  ?.message || '';
              console.log(
                '[Azure Foundry] First attempt failed:',
                firstErrorMessage
              );

              if (firstErrorMessage.includes('max_completion_tokens')) {
                console.log(
                  '[Azure Foundry] Retrying with max_tokens for older model'
                );
                response = await fetchWithTimeout(
                  testUrl,
                  {
      ...
      try {
        ...
        if (message.includes('AccessDeniedException')) {
          return {
            valid: false,
            error:
              'Access denied. Ensure your AWS credentials have Bedrock permissions.',
          };
        }
  ...
  handle(
    ...
    async (_event: IpcMainInvokeEvent, credentials: string) => {
      ...
      return {
        ...
        provider: 'bedrock',
        label:
          parsed.authType === 'accessKeys'
            ? 'AWS Access Keys'
            : `AWS Profile: ${parsed.profileName}`,
        keyPrefix:
          parsed.authType === 'accessKeys'
            ? `${parsed.accessKeyId.substring(0, 8)}...`
            : parsed.profileName,
        isActive: true,
        createdAt: new Date().toISOString(),
      };
      ...
      tools: [
        {
          ...
          function: {
            ...
            description:
              'Gets the current time. Must be called to know what time it is.',
            parameters: {
              type: 'object',
              properties: {
                timezone: {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          ...
          console.log(
            `[Ollama] Model ${modelId} does not support tools (error response)`
          );
          return 'unsupported';
        }
        console.warn(
          `[Ollama] Tool test failed for ${modelId}: ${response.status}`
        );
        return 'unknown';
      }
      ...
      if (choice?.message?.tool_calls && choice.message.tool_calls.length > 0) {
        console.log(
          `[Ollama] Model ${modelId} supports tools (made tool call)`
        );
        return 'supported';
      }
      ...
      console.log(
        `[Ollama] Model ${modelId} did not make tool call despite required - marking as unknown`
      );
      return 'unknown';
    } catch (error) {
      if (error instanceof Error) {
        ...
        if (
          ...
          console.log(
            `[Ollama] Model ${modelId} does not support tools (exception)`
          );
          return 'unsupported';
        }
      }
  ...
  handle(
    ...
    async (_event: IpcMainInvokeEvent, url: string) => {
      ...
      try {
        ...
        console.log(
          `[Ollama] Found ${rawModels.length} models, testing tool support...`
        );

        // Test tool support for each model
        const models: OllamaModel[] = [];
        for (const m of rawModels) {
          const toolSupport = await testOllamaModelToolSupport(
            sanitizedUrl,
            m.name
          );
          models.push({
    ...
    async (_event: IpcMainInvokeEvent, config: AzureFoundryConfig | null) => {
      if (config !== null) {
        ...
        try {
          ...
          if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
            throw new Error(
              'Invalid Azure Foundry configuration: Only http and https URLs are allowed'
            );
          }
        } catch {
          throw new Error(
  ...
  handle(
    ...
    async (
      ...
      try {
        ...
        console.log(
          '[Azure Foundry] Connection test successful for deployment:',
          deploymentName
        );
        return { success: true };
      } catch (error) {
        ...
        if (error instanceof Error && error.name === 'AbortError') {
          return {
            success: false,
            error:
              'Request timed out. Check your endpoint URL and network connection.',
          };
        }
  ...
  handle(
    ...
    async (_event: IpcMainInvokeEvent, url: string, apiKey?: string) => {
      ...
      const sanitizedApiKey = apiKey
        ? sanitizeString(apiKey, 'apiKey', 256)
        : undefined;
      ...
      try {
        ...
        console.log(
          `[LiteLLM] Connection successful, found ${models.length} models`
        );
        return { success: true, models };
      } catch (error) {
        const message =
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorText.includes('tool') || errorText.includes('function')) {
          console.log(
            `[LM Studio] Model ${modelId} does not support tools (error response)`
          );
          return 'unsupported';
        }
        console.warn(
          `[LM Studio] Tool test failed for ${modelId}: ${response.status}`
        );
        return 'unknown';
      }
      ...
      if (choice?.message?.tool_calls && choice.message.tool_calls.length > 0) {
        console.log(
          `[LM Studio] Model ${modelId} supports tools (made tool call)`
        );
        return 'supported';
      }
      ...
      if (choice?.finish_reason === 'tool_calls') {
        console.log(
          `[LM Studio] Model ${modelId} supports tools (finish_reason)`
        );
        return 'supported';
      }
      ...
      // This likely means the model doesn't actually support tools (just ignored the param)
      console.log(
        `[LM Studio] Model ${modelId} did not make tool call despite required - marking as unknown`
      );
      return 'unknown';
    } catch (error) {
      if (error instanceof Error) {
        ...
        if (
          ...
          console.log(
            `[LM Studio] Model ${modelId} does not support tools (exception)`
          );
          return 'unsupported';
        }
      }
  ...
  handle(
    ...
    async (_event: IpcMainInvokeEvent, url: string) => {
      ...
      try {
        ...
        console.log(
          `[LM Studio] Found ${rawModels.length} models, testing tool support...`
        );
        ...
        for (const m of rawModels) {
          ...
          // Test tool support
          const toolSupport = await testLMStudioModelToolSupport(
            sanitizedUrl,
            m.id
          );
        ...
        console.log(
          `[LM Studio] Connection successful, found ${models.length} models`
        );
        return { success: true, models };
      } catch (error) {
        const message =
    ...
    async (_event: IpcMainInvokeEvent, config: LMStudioConfig | null) => {
      if (config !== null) {
        ...
        if (config.models !== undefined) {
          if (!Array.isArray(config.models)) {
            throw new Error(
              'Invalid LM Studio configuration: models must be an array'
            );
          }
          for (const model of config.models) {
            if (
              ...
              throw new Error(
                'Invalid LM Studio configuration: invalid model format'
              );
            }
          }
        }
