...
const ALLOWED_API_KEY_PROVIDERS = new Set(['anthropic', 'openai', 'openrouter', 'google', 'xai', 'deepseek', 'zai', 'azure-foundry', 'custom', 'bedrock', 'litellm', 'elevenlabs']);
...
export function registerIPCHandlers(): void {
  ...
  // Speech-to-Text: Get API key for transcription (secure - only called from main process via IPC)
  handle('speech:get-api-key', async (_event: IpcMainInvokeEvent) => {
    const apiKey = getApiKey('elevenlabs');
    return apiKey || '';
  });
