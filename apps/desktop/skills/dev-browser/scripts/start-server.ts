...
const ACCOMPLISH_HTTP_PORT = parseInt(
  process.env.DEV_BROWSER_PORT || '9224',
  10
);
const ACCOMPLISH_CDP_PORT = parseInt(
  process.env.DEV_BROWSER_CDP_PORT || '9225',
  10
);
...
try {
  ...
  if (res.ok) {
    ...
    if (info.mode === 'extension') {
      console.log(
        'Found relay server running, killing to start launch server...'
      );
      try {
        if (process.platform === 'win32') {
          const output = execSync(
            `netstat -ano | findstr :${ACCOMPLISH_HTTP_PORT}`,
            { encoding: 'utf-8' }
          );
          ...
          const pid = execSync(`lsof -ti:${ACCOMPLISH_HTTP_PORT}`, {
            encoding: 'utf-8',
          }).trim();
          if (pid) {
            execSync(`kill -9 ${pid}`);
      ...
      console.log(
        `Launch server already running on port ${ACCOMPLISH_HTTP_PORT}`
      );
      process.exit(0);
    }
  }
...
try {
  if (process.platform === 'win32') {
    ...
    // Unix: use lsof
    const pid = execSync(`lsof -ti:${ACCOMPLISH_CDP_PORT}`, {
      encoding: 'utf-8',
    }).trim();
    if (pid) {
      console.log(
  ...
  try {
    ...
    if (isBrowserMissing && !retry) {
      ...
      try {
        ...
        console.log(
          'You may need to run manually: npx playwright install chromium'
        );
        process.exit(1);
      }
    }
